...
dx_dict = {'center':0.00015904069695 * 1000, 'lower':0.000136494725612 * 1000} # pixel width [mm]
dt = 0.001
...
plt.tight_layout()
fig.savefig(figfolder+'center_scatter_matrix.pdf')
...
#ax.flatten()[0].legend()
plt.tight_layout()
fig.savefig(figfolder+'center_kde_plots.pdf')
...
c_to_subplotcol_mapping = {'k':0,'b':1,'r':2}
ls_to_subplotrow_mapping = {'-':0,'--':1}
for ri,run in enumerate(trajs):
    for bubble_num in list(run.keys()):
        bubble = run[bubble_num]
        print(len(bubble))
        ax[ls_to_subplotrow_mapping[ls_mapping[ri]],c_to_subplotcol_mapping[c_mapping[ri]]].plot(bubble['x']*dx,bubble['y']*dx,color=c_mapping[ri],linestyle=ls_mapping[ri],alpha=0.5)
        ax[ls_to_subplotrow_mapping[ls_mapping[ri]],c_to_subplotcol_mapping[c_mapping[ri]]].invert_yaxis()
        
[a.set(aspect=1) for a in ax.flatten()]
[a.set(adjustable='box-forced') for a in ax.flatten()]

ax[0,0].set_ylabel('Center Third\ny [mm]')
ax[1,0].set_ylabel('Lower Third\ny [mm]')
[a.set_xlabel('x [mm]') for a in ax[1]]
ax[0,0].set_title('No induced turbulence')
ax[0,1].set_title('A = 5 mm, f = 5 Hz')
ax[0,2].set_title('A = 10 mm, f = 5 Hz')

plt.show()
plt.pause(0.1)
fig.savefig(figfolder+'center_trajectories.pdf')
...
for ri,run in enumerate(trajs):
    ...
    for bubble_num in list(run.keys()):
        ...
        bubble_diff = run_diff[bubble_num][run[bubble_num]['minor_axis_length']>0]#.rolling(window=3,center=True,min_periods=0).mean()
        #(bubble['major_axis_length']/bubble['minor_axis_length']).plot.kde(ax=ax,alpha=0.2,color=c_mapping[ri],linestyle=ls_mapping[ri])
        (bubble_diff['x'].abs()/bubble['filled_area']).plot.kde(ax=ax,alpha=0.2,color=c_mapping[ri],linestyle=ls_mapping[ri])
...
run_num=9
bubble_num=0
bubble = trajs[run_num][list(trajs[run_num].keys())[bubble_num]].iloc[5:-5]
bubble_diff = diffs[run_num][list(trajs[run_num].keys())[bubble_num]].iloc[5:-5]
...
f = np.fft.fft(bubble['major_axis_length']*dx)

ax.semilogy(freq[freq>0],np.absolute(f[freq>0]))


'''
KDE by timepoint
'''
cols_to_kde = ['filled_area','major_axis_length','x','diff_y','diff_x','orientation','diff_orientation','ecc','diff_x_abs']
fig,ax = plt.subplots(4,3,figsize=(10,8))
for A in list(cmap.keys()):
    for region in list(lsmap.keys()):
        ...
        for ix in bubbles_to_use.index:
            ...
            bubble = trajs[run_num][bubble_num].rolling(window=3,center=True,min_periods=0).mean()
            #bubble = bubble / bubble.mean()
            
            bubble['ecc'] = (bubble['major_axis_length']-bubble['minor_axis_length']) / ((bubble['major_axis_length']+bubble['minor_axis_length']))
            bubble['diff_x_abs'] = bubble['diff_x'].abs()
            bubble['orientation_abs'] = bubble['orientation'].abs()
            
            df = pd.concat([df,bubble])
        ...
        if df.empty==False:
            ...
            #plt.figure()
            pd.scatter_matrix(df[['x','y','diff_x_abs','diff_y','orientation_abs','ecc']])
