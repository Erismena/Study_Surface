...
#meta4 = pd.DataFrame()
#
#case_names = [r'piv_4pumps_axialView_2x2_z25cm_sunbathing_on100_off400',
#              r'piv_4pumps_2x2_honeycomb_sunbathing_on100_off400']
#offsets = [(-1080,-2048),(-1024,-1024)]
#parent_folders = [r'C:\Users\Luc Deike\data_comp3_C\180220\\',
#                  r'C:\Users\Luc Deike\data_comp3_C\180221\\']
...
#meta4['grid'] = [True, False]
#meta4['description'] = ['no grid','grid']


#meta4 = pd.DataFrame()
#
#case_names = [r'piv_test_fullView_fps1000_exposure500_fullDur_alwaysOn',
#              r'piv_test_fullView_fps1000_exposure500_fullDur_sunbathing_on050_off200']
#offsets = [(-1080,-1500),(-1024,-1500)]
#parent_folders = [r'C:\Users\Luc Deike\data_comp3_C\180218\\',
#                  r'C:\Users\Luc Deike\data_comp3_C\180218\\']
#diffusers = [False] * len(case_names)
#periods = [0,250]
#on_portions = [1,.2]
#
...
#meta4['on_portion'] = on_portions
#meta4['need2rotate'] = [False,False]
#meta4['grid'] = [True, True]
#meta4['description'] = ['grid','grid']
#
#meta = meta4
...
case_names = [r'piv_4pumps_stackedParallel_sunbathing_h093mm_L460mm_T10s_meanon100_meanoff400_fps4000',
              r'piv_4x4_center_sunbathing_on100_off400_fps1000']
offsets = [(-550,-500),(-1024,-1024)]
parent_folders = [r'C:\Users\Luc Deike\data_comp3_C\180123\\',
...
meta4['period'] = periods
meta4['on_portion'] = on_portions
meta4['need2rotate'] = [True,False]
meta4['grid'] = [False, False]
meta4['description'] = ['pump plane','center plane']
...
Figs = piv.PIVComparisonsFigures(1,3,figsize=(12.5,4),max_speed=0.2,vmin=-.5,vmax=0.5,legend_axi=2)
skip_ax=[]
...
for i in meta.index:
    ...
    Figs.add_rect(ai,-rect_size,-rect_size,rect_size*2,rect_size*2,color=color,ls=ls,)
    ...
    A.run_autocorr(ff,time,3000,[smaller_rows[0],smaller_rows[1],smaller_cols[0],smaller_cols[1]])
    
    C_dict[case_name] = A.C_avg
    lags_dict[case_name] = A.lags
    lags = A.lags
...
#Figs.save_figs(figfolder,'full_view_honeycomb')
...
#for fig in [fig_composite_compare1]:
    #fig.savefig(figfolder+'full_view_honeycombs_compositeVerticalCol.pdf')
...
for i in meta.index:
    ...
    ax.plot(meta.loc[i,'on_portion'],meta.loc[i,'u_rms_0'],'o',c=c,)
    ax.plot(meta.loc[i,'on_portion'],meta.loc[i,'u_rms_1'],'x',c=c,)
#    
#fig = plt.figure(figsize=(8,4))
#ax = fig.add_subplot(111)
#
...
fig_ffts,ax_ffts = plt.subplots(1,2,figsize=(10,7),sharex=True,sharey=True)
...
for i in meta.index:
    ...
    label = meta.loc[i,'description']
    
    C_avg = C_dict[case_name]
    lags = lags_dict[case_name]
    
    ax_ffts[0].plot(lags,np.cumsum(C_avg[:,0])*(lags[1]-lags[0]),label=label)
    ax_ffts[1].plot(lags,np.cumsum(C_avg[:,1])*(lags[1]-lags[0]))
    
ax_ffts[0].legend()
ax_ffts[0].set_title('x-direction fluctuations')
ax_ffts[1].set_title('y-direction fluctuations')
ax_ffts[0].set_ylabel('integral of eulerian autocorrelation [s]')
ax_ffts[0].set_xlabel('lag [s]')
ax_ffts[1].set_xlabel('lag [s]')


ax_ffts[0].set_ylabel('$\int_0^t \rho_{ii}(\tau) d \tau$')
#ax_ffts[0].set_xlabel('$t$ [s]')
#ax_ffts[1].set_xlabel('$t$ [s]')
