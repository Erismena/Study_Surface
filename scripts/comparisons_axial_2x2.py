...
Figs = piv.PIVComparisonsFigures(1,3,figsize=(12.5,2.5),max_speed=0.15,vmin=vmin,vmax=vmax,legend_axi=2)
skip_ax=[]
...
for i in meta.index:
    ...
    lim = 0.03
    center_rows,center_cols = g.get_coords(np.array([[0.005,-.005],[-.005,0.005]]))
    ...
    Figs.update_limits(ai,[-0.18,0.18],[-0.08,0.08])
...
#Figs.save_figs(figfolder,'axial')
...
c_grids = {'no grid':'k','grid':'b'}

[ax1.plot(np.nan,np.nan,label=key,color=c_grids[key]) for key in list(c_grids.keys())]
ax1.legend()

for i in meta.index:
    t = radial_vals[i]
    color = c_grids[meta.loc[i,'description']]
...
fig.tight_layout()
fig.savefig(figfolder+'axial_spatial_scales.pdf')
