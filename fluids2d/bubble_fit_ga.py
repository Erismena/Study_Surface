...
class Ellipse:
    def __init__(self,params,nx,ny,lims):
        self.params = params
...
class Population:
    ...
    def mutate(self,lims,portion=.3,n_params=3):
        ...
        for mi in members_to_mutate:
            self.members[mi].mutate(lims,n=n_params)
    ...
    def reset_some(self,nx,ny,lims,portion=.4):
        members_to_reset = range(0,int(portion*len(self.members)))
        for mi in members_to_reset:
            self.members[mi] = Member([Ellipse(None,nx,ny,lims) for _ in range(len(self.members[mi].ellipses))])
...
def fit_subimage_with_ga(im_small,im_small_orig,n_bubbles=2):
    ...
    members = []
    for _ in range(200):            
        members.append(Member([Ellipse(None,nx,ny,lims) for _ in range(n_bubbles)]))    
